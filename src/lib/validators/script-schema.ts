...
export const scriptStepSchema = z
  ...
  .superRefine((val, ctx) => {
    if (val.type === 'inline' && !val.command) {
      ctx.addIssue({ code: 'custom', path: ['command'], message: 'Inline step requires command' })
    }
    if (val.type === 'command' && !val.command_id) {
      ctx.addIssue({ code: 'custom', path: ['command_id'], message: 'Command step requires command_id' })
    }
  })
